...
void V8StepperBridge::GetAttrLabel(
    v8::Local<v8::Context> ctx, v8::Local<v8::Value> valObject, StepperLabels& stepperLabel)
{
    ...
    for (int32_t i = 0; i < len; i++) {
        ...
        if (val->IsNumber() || val->IsBoolean() || val->IsString()) {
            ...
            if (strcmp(keyStr, DOM_STEPPER_LEFT_LABEL) == 0) {
                stepperLabel.leftLabel = valStr;
            } else if (strcmp(keyStr, DOM_STEPPER_RIGHT_LABEL) == 0) {
                stepperLabel.rightLabel = valStr;
            } else if (strcmp(keyStr, DOM_STEPPER_INITIAL_STATUS) == 0) {
