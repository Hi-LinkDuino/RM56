...
void TransitionImpl::ScaleCenterPointer(int16_t scale){
    double value = (double)scale;
    value = value /ALPHA_MAX;
    TransformMap transMap(view_->GetOrigRect());
    float halfVal = 2.0f;
    transMap.Scale(Vector2<float>(value, value), Vector2<float>((view_->GetWidth() - 1) / halfVal, (view_->GetHeight() - 1) / halfVal));
    view_->SetTransformMap(transMap);
}
                                       ...
                                       int32_t elapsedTime)
{
    ...
    switch (transitionType) {
        ...
        case TransitionType::TSCALE:{
            ScaleCenterPointer(updateAttrValue);
            break;
        }
