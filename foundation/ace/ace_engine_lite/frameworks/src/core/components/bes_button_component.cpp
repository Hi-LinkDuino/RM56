...
bool BesButtonComponent::CreateNativeViews()
{
...
void BesButtonComponent::ReleaseNativeViews()
{
...
bool BesButtonComponent::SetPrivateAttribute(uint16_t attrKeyId, jerry_value_t attrValue)
{
    bool result = false;
    switch (attrKeyId) {
        case K_VALUE:
        ...
        case K_BACKGROUND_IMAGE:
            ACE_FREE(normalBackGroundImg_);
            normalBackGroundImg_ = const_cast<char *>(ParseImageSrc(attrValue));
            break;
        case K_BES_PRESS_IMAGE:
            ACE_FREE(pressedBackGroundImg_);
            pressedBackGroundImg_ = const_cast<char *>(ParseImageSrc(attrValue));
            break;
        default:
            return false;
    }
...
bool BesButtonComponent::ApplyPrivateStyle(const AppStyleItem *style)
{
    bool result = true;
    ...
    switch (styleKey) {
        ...
        case K_COLOR:
            ...
            break;
        case K_BES_PRESS_COLOR:
            result = SetPressColor(*style);
            break;
...
bool BesButtonComponent::SetPressColor(const AppStyleItem &style){
    uint32_t color = 0;
...
bool BesButtonComponent::SetBackgroundColor(const AppStyleItem &style)
{
...
bool BesButtonComponent::SetColor(const AppStyleItem &style)
{
    uint32_t color = 0;
    uint8_t alpha = OPA_OPAQUE;
    if (GetStyleColorValue(&style, color, alpha)) {
...
bool BesButtonComponent::SetTextAlign(const AppStyleItem *style)
{
...
bool BesButtonComponent::SetFontSize(const AppStyleItem *style)
{
    fontSize_ = GetStylePixelValue(style);
    return true;
}

void BesButtonComponent::PostRender()
{
...
void BesButtonComponent::PostUpdate(uint16_t attrKeyId)
{
