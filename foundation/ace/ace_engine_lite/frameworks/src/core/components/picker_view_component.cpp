...
void PickerViewComponent::SetLoopState(){
    if(pickerType_ == PickerType::UNKNOWN){
        HILOG_ERROR(HILOG_MODULE_ACE, "PickerViewComponent: picker view type not init");
        return;
    }
    if (pickerView_ == nullptr) {
        HILOG_ERROR(HILOG_MODULE_ACE, "PickerViewComponent: pickerView_ is nullptr");
        return;
    }
     switch (pickerType_) {
        case PickerType::TEXT: {
            UIPickerTypeWrapper *uiPicker = static_cast<UIPickerTypeWrapper *>(pickerView_);
            uiPicker->SetLoopState(isLoop_);
            break;
        }
        case PickerType::TIME: {
            UITimePicker *uiTimePicker= static_cast<UITimePicker *>(pickerView_);
            uiTimePicker->SetLoopState(uiTimePicker->PICKER_HOUR, isLoop_);
            uiTimePicker->SetLoopState(uiTimePicker->PICKER_MIN, isLoop_);
            uiTimePicker->SetLoopState(uiTimePicker->PICKER_SEC, isLoop_);
            break;
        }
#if (FEATURE_COMPONENT_DATE_PICKER == 1)
        case PickerType::DATE: {
...
bool PickerViewComponent::ApplyPrivateStyle(const AppStyleItem *styleItem)
{
    ...
    switch (styleNameId) {
        case K_BES_LOOP:
            isLoop_ = styleItem->GetBoolValue();
            SetLoopState();
            break;
        case K_COLOR:
...
bool PickerViewComponent::SetTextPickerRange(jerry_value_t rangeValue)
{
    ...
    if (textArraySize_ == 1) {
        ...
        textPicker->SetLoopState(isLoop_);
    }
    return true;
}
} // namespace ACELite
