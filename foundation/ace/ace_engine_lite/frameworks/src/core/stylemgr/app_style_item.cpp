...
//yongxianglai@bestechnic.com: 先于默认的处理流程
bool AppStyleItem::ProcessPrivateKeyId(const uint16_t keyId, const jerry_value_t stylePropValue, AppStyleItem *newStyleItem)
{
    if (jerry_value_is_number(stylePropValue)) {
        if (keyId == K_BES_SCROLLBAR_LEN_FACTOR) {
...
AppStyleItem *AppStyleItem::CreateStyleItem(uint16_t keyId, const jerry_value_t stylePropValue, uint8_t pseudoClassType)
{
    ...
    //yongxianglai@bestechnic.com: 如果命中自定义的 keyId，不再走默认的解析逻辑，以让 jsui 端设置的自定义字段类型能被正确处理
    bool isProcessed = ProcessPrivateKeyId(keyId, stylePropValue, newStyleItem);
    if (isProcessed) return newStyleItem;
