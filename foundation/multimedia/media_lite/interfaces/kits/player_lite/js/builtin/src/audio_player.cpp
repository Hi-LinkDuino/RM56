...
{
    player_ = OHOS::Media::CreateHiPlayer();
    if (player_ == nullptr) {
...
{
    MEDIA_INFO_LOG("audioPlayer destory enter");
    if (player_ != nullptr) {
        if(player_->IsPlaying())
        {
            MEDIA_INFO_LOG("now is playing");
            if (player_->Stop() != 0) {
    ...
    ReleaseSrc();
    status_ = const_cast<char *>(STATUS_STOP);
    MEDIA_INFO_LOG("audioPlayer destory exit");
    return true;
}
...
bool AudioPlayer::ResetPlayer()
{
    ...
    MEDIA_INFO_LOG("audioPlayer Reset enter");
    ...
    (void)pthread_mutex_unlock(&lock_);
    MEDIA_INFO_LOG("audioPlayer Reset exit");
    return ret;
}
...
{
    MEDIA_INFO_LOG("audioPlayer stop");
...
{
    if (src_ != nullptr) {
        MEDIA_ERR_LOG("SetSrc and player_ not null");
        if (!DestoryPlayer()) {
            MEDIA_ERR_LOG("fail to destory player.");
        }
    }
