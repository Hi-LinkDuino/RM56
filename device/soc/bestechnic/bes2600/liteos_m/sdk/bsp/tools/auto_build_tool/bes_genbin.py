...
# argv: best2003 16m_flash|32m_flash
...
if len(argv_list) == 1:
    if argv_list[0] != 'best2001' and argv_list[0] != 'best2003':
        raise RuntimeError(external_para_format)
elif len(argv_list) == 2:
    if argv_list[0] != 'best2003':
        raise RuntimeError(external_para_format)
    elif argv_list[1] == '16m_flash' or argv_list[1] == '32m_flash':
        M_flash = argv_list[1]
    else:
        raise RuntimeError(external_para_format)
else:
...
if True:
    ...
    if isExists == True:
        os.system('rm -rf ' + release_file_dir)
    # ../write_flash_gui/ota_bin
    isExists = os.path.exists(gui_bin_dir)
    if isExists == True:
        os.system('rm -rf ' + gui_bin_dir)
    ...
    # ../write_flash_gui_best2001.tar.gz
    best2001_tar_dir = '../' + pack_file_name_best2001
    isExists = os.path.exists(best2001_tar_dir)
    if isExists == True:
        os.system('rm -rf ' + best2001_tar_dir)
    # ../write_flash_gui_best2003_16m_flash.tar.gz
    best2003_16m_flash_tar_dir = '../' + pack_file_name_best2003_16m_flash
    isExists = os.path.exists(best2003_16m_flash_tar_dir)
    if isExists == True:
        os.system('rm -rf ' + best2003_16m_flash_tar_dir)
    # ../write_flash_gui_best2003_32m_flash.tar.gz
    best2003_32m_flash_tar_dir = '../' +  pack_file_name_best2003_32m_flash
    isExists = os.path.exists(best2003_32m_flash_tar_dir)
    if isExists == True:
        os.system('rm -rf ' + best2003_32m_flash_tar_dir)
    # print("Folders creation done.\n")
...
# makedir release_bin ota_bin
if True:
    print("Start creating release_bin and write_flash_gui/ota_bin folders...\n")
    # ../release_bin
    isExists=os.path.exists(release_file_dir)
    if isExists == False:
...
if True:
    print('Start copy unsigned bin files from prebuild/*.bin to release_bin...')
    # os.system('chmod 777 ' + release_file_dir + '/')
    path = sign_file_dir
    # Returns a list of files or folder names that the specified folder contains.
    file_list = []
    ...
    for i in range(len(file_list)):
        ...
        if result != 0:
            raise RuntimeError(cmd_str + ' failed')
    print("Copy prebuild/*.bin to release_bin done.\n\n" )


#  cp sign_file_out_list_bin  release_bin
if True:
    print('Start copy unsigned bin files from sign_file_out_list to release_bin...')
    dict_sign_out_list = cfg_dict['SIGN_FILE_OUT_LIST']
    print(dict_sign_out_list)
    list_items_file = dict_sign_out_list
    file_itme_len = len(list_items_file)
    for index in range(file_itme_len):
        file_item = list_items_file[index]
        print('\n',file_item)
        ren_bin_name = file_item['REN_BIN_NAME']
        binary_dir = file_item['BINARY_DIR']
        file_name = file_item['FILE_NAME']
        res_file_path = binary_dir + '/' + file_name
        des_path = release_file_dir + '/'
        #  cp sign_file_list_bin  release_bin
        cmd_str = 'cp -f ' + res_file_path + ' ' + des_path
        if log_switch:
            print(cmd_str)
        result = os.system(cmd_str)
        if result != 0:
            raise RuntimeError(cmd_str + ' failed')
        # mv release_bin/****.bin release_bin/res_bin_name.bin
        if ren_bin_name != file_name:
            res_file_path = release_file_dir + '/' + file_name
            des_file_path = release_file_dir + '/' + ren_bin_name
            ...
            if result != 0:
                raise RuntimeError(cmd_str + ' failed')
    print('\nCopy bin files from SIGN_FILE_OUT_LIST to release_bin done.\n\n')


# cp ../write_flash_gui/dld_cfg/bes_dld_cfg.yaml to ../write_flash_gui/
if True:
    print('Start coping dld_cfg yaml file...')
    res_file_path = dld_cfg_dir + '/' + pro_dld_cfg_file
    des_path = gui_tools_dir + '/'
    cmd_str = 'cp -f ' + res_file_path + ' ' + des_path
    isExists = os.path.exists(res_file_path)
    if isExists == True:
        if log_switch:
            print(cmd_str)
        result = os.system(cmd_str)
        ...
        raise RuntimeError('%s : No such file or directory. %s overwrites failed.\n' % (res_file_path,pro_dld_cfg_file))

# mv ../write_flash_gui/dld_cfg/bes_dld_cfg_*.yaml to ../write_flash_gui/bes_dld_cfg.yaml
if True:
    if pro_dld_cfg_file != dld_cfg_file:
        ...
        if result != 0:
            raise RuntimeError(cmd_str + ' failed')
    print('The dld_cfg file overwrites to write_flash_gui done.\n\n')

# define signature function
def bin_sign_fun(dict_sign_file,sign_bin):
...
sign_bin = False
if True:
    # sign bin in list SIGN_FILE_OUT_LIST
    print('Start signing bin in list SIGN_FILE_OUT_LIST...')
    dict_sign_file = cfg_dict['SIGN_FILE_OUT_LIST']
    sign_bin = bin_sign_fun(dict_sign_file, sign_bin)
...
# mv release_bin/programmer*.bin   write_flash_gui/
if True:
    print('Start moving %s to write_flash_gui...' % (programmer_name))
    res_file_path = release_file_dir + '/' + programmer_name
    isExists = os.path.exists(res_file_path)
    if isExists == True:
        ...
        result = os.system(cmd_str)
        if result != 0:
            raise RuntimeError("%s move failed." % (programmer_name))
        print("%s move done.\n" % (programmer_name))
    else:
        raise RuntimeError('%s : No such file or directory. %s move failed.\n'%(res_file_path,programmer_name))



# cp release_bin/*.bin ota_bin
if True:
    print('Start coping release_bin file to ota_bin...')
    ...
    for i in range(len(file_list)):
        ...
        if result != 0:
            raise RuntimeError(cmd_str + ' failed.')
    print("release_bin file copy to ota_bin done.\n")
    if sign_bin:
        print('bes sign done.\n\n')
    else:
        print('The bin files packaging done.\n\n')


# Pack write_flash_gui
if True:
    print('Pack file start...')
    cur_dir = os.getcwd()
    pre_dir = os.path.abspath(os.path.dirname(os.getcwd()))
    os.chdir(pre_dir)
    to_pack_file_dir = gui_tools_dir[3:]
    cmd_str = 'tar -zcvf ' + pack_file_name + ' ' + to_pack_file_dir
    result = os.system(cmd_str)
    if result != 0:
        raise RuntimeError('Pack ' + pack_file_name + ' file failed.\n')
    os.chdir(cur_dir)
    print('Pack ' + pack_file_name + ' file done.\n')
