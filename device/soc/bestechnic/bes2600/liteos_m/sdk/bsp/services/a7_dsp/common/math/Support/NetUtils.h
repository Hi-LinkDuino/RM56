...
//---------------------------------------------------------------------------------------------------------------------------
/*!	@function	ServerSocketPairOpen
	@abstract	Opens two listening sockets, one for IPv4 and one for IPv6. See ServerSocketOpen.
*/
OSStatus
...
//---------------------------------------------------------------------------------------------------------------------------
/*!	@function	ServerSocketOpen
	@abstract	Opens a listening socket with the specified options
*/
OSStatus
ServerSocketOpen(
    int inFamily,
...
#else
#define SUN_LEN_MAX 104
#endif

//---------------------------------------------------------------------------------------------------------------------------
/*!	@function	ServerSocketOpenUnix
	@abstract	Opens a listening socket with the specified options in the Unix domain.
 
	@param		inType			Socket type. Most platforms support SOCK_STREAM, SOCK_DGRAM and SOCK_SEQPACKET
	@param		inPath			Path the socket is bound to.
	@param		inPathLen		Length of inPath. Must be < SUN_LEN_MAX
	@param		inUnlink		Attempt to unlink the file at inPath before binding the socket.
	@param		inRcvBufSize	Size of the receive buffer of accepted sockets.
	@param		outSock			The destination socket.
*/
OSStatus
ServerSocketOpenUnix(
    int inType,
    char* inPath,
    size_t inPathLen,
    Boolean inUnlink,
...
//---------------------------------------------------------------------------------------------------------------------------
/*!	@function	UNIXConnect
	@abstract	Connect to the socket specified by the given path
 
	@param		inPath			Path the socket is bound to.
	@param		inPathLen		Length of inPath. Must be < SUN_LEN_MAX
	@param		inType			Socket type. Most platforms support SOCK_STREAM, SOCK_DGRAM and SOCK_SEQPACKET
	@param		inSeconds		Timeout to wait for the connection to succeed in seconds.
	@param		outSock			The destination socket.
*/
OSStatus
UNIXConnect(
...
//---------------------------------------------------------------------------------------------------------------------------
/*!	@function	SockAddrInitUnix
	@abstract	Formats a sockaddr_un with the specified payload
 
	@param		inPath			Path the socket is bound to.
	@param		inPathLen		Length of inPath. Must be < SUN_LEN_MAX
	@param		outSockAddr		The destination sockaddr_un.
*/
OSStatus
SockAddrInitUnix(
    char* inPath,
    size_t inPathLen,
    struct sockaddr_un* outSockAddr);
...
/*!	@function	SocketAccept
	@abstract	Accepts a connection on a socket with a timeout. Only supports IPv4/IPv6 sockets.
...
//---------------------------------------------------------------------------------------------------------------------------
/*!	@function	SocketAccept2
	@abstract	Accepts a connection on a socket with a timeout. Supports IPv4/IPv6/Unix domain sockets.
	...
	@param		outAddr		Receives sockaddr of connected peer. May be NULL.
*/
OSStatus SocketAccept2(SocketRef inSock, int inSeconds, SocketRef* outSock, sockaddr_iu* outAddr);
...
//---------------------------------------------------------------------------------------------------------------------------
/*!	@function	SocketGetType
	@abstract	Gets the type (e.g. SOCK_STREAM) of a socket.
*/
int SocketGetType(SocketRef inSock, OSStatus* outErr);
