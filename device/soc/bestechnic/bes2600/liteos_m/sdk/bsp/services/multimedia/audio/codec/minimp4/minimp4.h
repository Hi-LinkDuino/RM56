...
#define MP4D_TIMESTAMPS_SUPPORTED 0

// Enable TrackFragmentBaseMediaDecodeTimeBox support
#define MP4D_TFDT_SUPPORT         0

// add those to stat minimp4 memory usage.
#if MP4D_TRACE_SUPPORTED
#define minimp4_malloc(n) \
    malloc(n);            \
    printf("minimp4 malloc. %s %d. size = %d\n", __func__, __LINE__, n);
#define minimp4_realloc(p, n) \
    realloc(p, n);            \
    printf("minimp4 realloc. %s %d. size = %d\n", __func__, __LINE__, n);
#define minimp4_free(p) \
    free(p);            \
    printf("minimp4 free. %s %d.\n", __func__, __LINE__);
#else
#define minimp4_malloc(n) \
    malloc(n)
#define minimp4_realloc(p, n) \
...
static int minimp4_vector_init(minimp4_vector_t *h, int capacity)
{
    ...
    if(capacity)
    {
        h->data = (unsigned char *)minimp4_malloc(capacity);
    }
    else
    {
        h->data = NULL;
...
#if MP4D_TRACE_SUPPORTED
#include "hal_trace.h"
#   define MINMP4_TRACE(x) printf x
#else
#   define MINMP4_TRACE(x)
#endif
                ...
                if (box_name == g_fullbox[i].name)
                {
                    ...
                    if (box_name == BOX_meta)
                    {
                        if (FullAtomVersionAndFlags >= 8 &&  FullAtomVersionAndFlags < payload_bytes)
                        {
                            ...
                            MINMP4_TRACE(("Bad metadata box detected (Android bug?)!\n"));
        ...
        case BOX_stts:
            {
                ...
                for (i = 0; i < count; i++)
                {
                    unsigned sc = READ(4);
                    int d =  READ(4);
                    MINMP4_TRACE(("MINMP4_2867: sample %8d count %8d duration %8d\n", i, sc, d));
#if MP4D_TIMESTAMPS_SUPPORTED
                    if (k + sc > ts_count)
                    {
                        ts_count = k + sc;
        ...
        case BOX_ctts:
            {
                ...
                for (i = 0; i < count; i++)
                {
                    ...
                    (void)sc;
                    (void)d;
                    MINMP4_TRACE(("sample %8d count %8d decoding to composition offset %8d\n", i, sc, d));
                }
            }
            break;
#endif
...
//         case BOX_svc1:   - no test
        ...
        if (ptag && !*ptag && payload_bytes > 16)
        {
            ...
            uint32_t x1 = READ(4);
            MINMP4_TRACE(("%2d  %2d %2d ", size, class, x1));
#else
            SKIP(4 + 4 + 4 + 4);
#endif
        ...
        if (box_name == BOX_trak)
        {
            ...
            if (!mem)
            {
                // if minimp4_realloc fails, it does not deallocate old pointer!
                ERROR("out of memory");
            }
