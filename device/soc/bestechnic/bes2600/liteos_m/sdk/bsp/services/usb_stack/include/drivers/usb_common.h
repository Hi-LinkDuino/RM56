...
#define RT_DEBUG_USB                    0x01 // enable usb debug
...
rt_inline void print_udev_desc(const char *name, udev_desc_t desc)
{
    RT_ASSERT(desc != RT_NULL);
    rt_kprintf("%s\n-----USB_DEVICE_DESCRIPTOR-----\n", name);
    rt_kprintf("bLength             = 0x%X\n", desc->bLength);
    rt_kprintf("type                = 0x%X\n", desc->type);
    rt_kprintf("bcdUSB              = 0x%X\n", desc->bcdUSB);
    rt_kprintf("bDeviceClass        = 0x%X\n", desc->bDeviceClass);
    rt_kprintf("bDeviceSubClass     = 0x%X\n", desc->bDeviceSubClass);
    rt_kprintf("bDeviceProtocol     = 0x%X\n", desc->bDeviceProtocol);
    rt_kprintf("bMaxPacketSize0     = 0x%X\n", desc->bMaxPacketSize0);
    rt_kprintf("idVendor            = 0x%X\n", desc->idVendor);
    rt_kprintf("idProduct           = 0x%X\n", desc->idProduct);
    rt_kprintf("bcdDevice           = 0x%X\n", desc->bcdDevice);
    rt_kprintf("iManufacturer       = 0x%X\n", desc->iManufacturer);
    rt_kprintf("iProduct            = 0x%X\n", desc->iProduct);
    rt_kprintf("iSerialNumber       = 0x%X\n", desc->iSerialNumber);
    rt_kprintf("bNumConfigurations  = 0x%X\n", desc->bNumConfigurations);
}
...
struct uconfig_descriptor
{
    ...
    rt_uint8_t MaxPower;
    rt_uint8_t data[256]; // 2048 in rtt4.0.4
};
typedef struct uconfig_descriptor* ucfg_desc_t;
rt_inline void print_ucfg_desc(const char *name, ucfg_desc_t desc)
{
    RT_ASSERT(desc != RT_NULL);
    rt_kprintf("%s\n-----USB_CONFIGURATION_DESCRIPTOR-----\n", name);
    rt_kprintf("bLength             = 0x%X\n", desc->bLength);
    rt_kprintf("type                = 0x%X\n", desc->type);
    rt_kprintf("wTotalLength        = 0x%X\n", desc->wTotalLength);
    rt_kprintf("bNumInterfaces      = 0x%X\n", desc->bNumInterfaces);
    rt_kprintf("bConfigurationValue = 0x%X\n", desc->bConfigurationValue);
    rt_kprintf("iConfiguration      = 0x%X\n", desc->iConfiguration);
    rt_kprintf("bmAttributes        = 0x%X\n", desc->bmAttributes);
    rt_kprintf("MaxPower            = 0x%X\n", desc->MaxPower);
}
...
rt_inline void print_uintf_desc(const char *name, uintf_desc_t desc)
{
    RT_ASSERT(desc != RT_NULL);
    rt_kprintf("%s\n-----USB_INTERFACE_DESCRIPTOR-----\n", name);
    rt_kprintf("bLength             = 0x%X\n", desc->bLength);
    rt_kprintf("type                = 0x%X\n", desc->type);
    rt_kprintf("bInterfaceNumber    = 0x%X\n", desc->bInterfaceNumber);
    rt_kprintf("bAlternateSetting   = 0x%X\n", desc->bAlternateSetting);
    rt_kprintf("bNumEndpoints       = 0x%X\n", desc->bNumEndpoints);
    rt_kprintf("bInterfaceClass     = 0x%X\n", desc->bInterfaceClass);
    rt_kprintf("bInterfaceSubClass  = 0x%X\n", desc->bInterfaceSubClass);
    rt_kprintf("bInterfaceProtocol  = 0x%X\n", desc->bInterfaceProtocol);
    rt_kprintf("iInterface          = 0x%X\n", desc->iInterface);
}
...
rt_inline void print_uiad_desc(const char *name, uiad_desc_t desc)
{
    RT_ASSERT(desc != RT_NULL);
    rt_kprintf("%s\n-----USB_IAD_DESCRIPTOR-----\n", name);
    rt_kprintf("bLength             = 0x%X\n", desc->bLength);
    rt_kprintf("bDescriptorType     = 0x%X\n", desc->bDescriptorType);
    rt_kprintf("bFirstInterface     = 0x%X\n", desc->bFirstInterface);
    rt_kprintf("bInterfaceCount     = 0x%X\n", desc->bInterfaceCount);
    rt_kprintf("bFunctionClass      = 0x%X\n", desc->bFunctionClass);
    rt_kprintf("bFunctionSubClass   = 0x%X\n", desc->bFunctionSubClass);
    rt_kprintf("bFunctionProtocol   = 0x%X\n", desc->bFunctionProtocol);
    rt_kprintf("iFunction           = 0x%X\n", desc->iFunction);
}
...
rt_inline void print_uep_desc(const char *name, uep_desc_t desc)
{
    RT_ASSERT(desc != RT_NULL);
    rt_kprintf("%s\n-----USB_ENDPOINT_DESCRIPTOR-----\n", name);
    rt_kprintf("bLength             = 0x%X\n", desc->bLength);
    rt_kprintf("type                = 0x%X\n", desc->type);
    rt_kprintf("bEndpointAddress    = 0x%X\n", desc->bEndpointAddress);
    rt_kprintf("bmAttributes        = 0x%X\n", desc->bmAttributes);
    rt_kprintf("wMaxPacketSize      = 0x%X\n", desc->wMaxPacketSize);
    rt_kprintf("bInterval           = 0x%X\n", desc->bInterval);
}
...
rt_inline void print_ustr_desc(const char *name, ustr_desc_t desc)
{
    RT_ASSERT(desc != RT_NULL);
    ...
    for (int i=0; i<desc->bLength - 2; i+=2) {
        if ((0x30 <= desc->String[i] && desc->String[i] <= 0x39) || 
            (0x41 <= desc->String[i] && desc->String[i] <= 0x46))
            rt_kprintf("%c ", desc->String[i]);
        else 
            rt_kprintf("0x%X ", desc->String[i]);
    }
    rt_kprintf("\n");
}
...
rt_inline void print_uhub_desc(const char *name, uhub_desc_t desc)
{
    RT_ASSERT(desc != RT_NULL);
    rt_kprintf("%s\n-----USB_HUB_DESCRIPTOR-----\n", name);
    rt_kprintf("length          = 0x%X\n", desc->length);
    rt_kprintf("type            = 0x%X\n", desc->type);
    rt_kprintf("num_ports       = 0x%X\n", desc->num_ports);
    rt_kprintf("characteristics = 0x%X\n", desc->characteristics);
    rt_kprintf("pwron_to_good   = 0x%X\n", desc->pwron_to_good);
    rt_kprintf("current         = 0x%X\n", desc->current);
}
