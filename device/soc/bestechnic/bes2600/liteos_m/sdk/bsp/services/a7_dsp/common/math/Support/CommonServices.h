/*
	Copyright (C) 2002-2021 Apple Inc. All Rights Reserved. Not to be used or disclosed without permission from Apple.
...
// sockaddr_iu -- like sockaddr_ip, but also allows for sockets in the unix domain.
...
typedef union {
    ...
    sockaddr_ip ip;
#if (defined(AF_UNIX))
    struct sockaddr_un un; // If the platform doesn't support AF_UNIX, clients should still be able to use sockaddr_un in non-UNIX domain-specific APIs
#endif
} sockaddr_iu;
#endif
...
#elif (TARGET_OS_POSIX)
#define MinimalMutexDefine(NAME) static mutex_handle_t NAME = MUTEX_CREATEIALIZER
#define MinimalMutexEnsureInitialized(NAME) \
    do {                                    \
    } while (0)
#define MinimalMutexEnsureFinalized(NAME) \
