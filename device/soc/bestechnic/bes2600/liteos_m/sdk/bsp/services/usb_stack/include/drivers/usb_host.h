...
rt_err_t rt_usbh_attach_instance(struct uinstance* device);
rt_err_t rt_usbh_detach_instance(struct uinstance* device);
rt_err_t rt_usbh_get_descriptor(struct uinstance* device, rt_uint8_t type, rt_uint8_t index, void* buffer, int nbytes);
...
ucd_t rt_usbh_class_driver_ecm_comm(void);
ucd_t rt_usbh_class_driver_ecm_data(void);
...
rt_inline rt_err_t rt_usb_hcd_alloc_pipe(uhcd_t hcd, upipe_t* pipe, uinst_t inst, uep_desc_t ep)
{
    *pipe = (upipe_t)rt_malloc(sizeof(struct upipe));
    ...
    if(hcd->ops->open_pipe(*pipe) != RT_EOK)
    {
        free(*pipe);
        *pipe = RT_NULL;
        return RT_ERROR;
...
rt_inline rt_uint8_t rt_usbh_get_config_ops(uinst_t device)
{
    RT_ASSERT(device != RT_NULL);
    if (device->hcd->ops->get_config)
        return device->hcd->ops->get_config();
    return 1;
}
#ifdef __cplusplus
}
