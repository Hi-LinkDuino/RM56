#ifndef __GATT_SERVER_INTERFACE_H__
#define __GATT_SERVER_INTERFACE_H__
...
typedef struct {
    ...
    wh_bt_status_t (*register_server)( bt_uuid_t *uuid );

    /** Unregister a server application from the stack */
    wh_bt_status_t (*unregister_server)(int server_if );

    /** Create a connection to a remote peripheral */
    wh_bt_status_t (*connect)(int server_if, const bt_bdaddr_t *bd_addr,
                            bool is_direct, int transport);

    /** Disconnect an established connection or cancel a pending one */
    wh_bt_status_t (*disconnect)(int server_if, const bt_bdaddr_t *bd_addr,
                    int conn_id );

    /** Create a new service */
    wh_bt_status_t (*add_service)( int server_if, btgatt_srvc_id_t *srvc_id, int num_handles);

    /** Assign an included service to it's parent service */
    wh_bt_status_t (*add_included_service)( int server_if, int service_handle, int included_handle);

    /** Add a characteristic to a service */
    wh_bt_status_t (*add_characteristic)( int server_if,
                    int service_handle, bt_uuid_t *uuid,
                    int properties, int permissions);

    /** Add a descriptor to a given service */
    wh_bt_status_t (*add_descriptor)(int server_if, int service_handle,
                                  bt_uuid_t *uuid, int permissions);

    /** Starts a local service */
    wh_bt_status_t (*start_service)(int server_if, int service_handle,
                                 int transport);

    /** Stops a local service */
    wh_bt_status_t (*stop_service)(int server_if, int service_handle);

    /** Delete a local service */
    wh_bt_status_t (*delete_service)(int server_if, int service_handle);

    /** Send value indication to a remote device */
    wh_bt_status_t (*send_indication)(int server_if, int attribute_handle,
                                   int conn_id, int len, int confirm,
                                   char* p_value);

    /** Send a response to a read/write operation */
    wh_bt_status_t (*send_response)(int conn_id, int trans_id,
