...
struct ieee80211_sta {
	...
	struct sta_info * info; /* added for qos on/off by tx status, 20200517 */
		      ...
		      const struct ieee80211_tx_info *c)
{
	if (WARN_ON(c->control.rates[0].idx < 0))
		return NULL;
	return &hw->wiphy->bands[c->band]->bitrates[(short)(c->control.rates[0].idx)];
}
			   ...
			   const struct ieee80211_tx_info *c)
{
	if (c->control.rts_cts_rate_idx < 0)
		return NULL;
	return &hw->wiphy->bands[c->band]->bitrates[(short)(c->control.rts_cts_rate_idx)];
}
			     ...
			     const struct ieee80211_tx_info *c, int idx)
{
	if (c->control.rates[idx + 1].idx < 0)
		return NULL;
	return &hw->wiphy->bands[c->band]->bitrates[(short)(c->control.rates[idx + 1].idx)];
}
...
static inline bool
ieee80211_is_dhcp_pkt(struct sk_buff *skb)
{
	if (skb->protocol == cpu_to_be16(ETH_P_IP)) {
		...
		if (iph->protocol == IPPROTO_UDP) { // UDP
			...
			if ((udph->source == __constant_htons(CLIENT_PORT))
			    && (udph->dest == __constant_htons(SERVER_PORT))) { // DHCP client->>server
				return true;
			}
		}
	}
