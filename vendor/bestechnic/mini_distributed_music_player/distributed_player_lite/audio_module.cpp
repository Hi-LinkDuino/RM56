...
extern "C" bool WriteInt32(IpcIo *io, int32_t value);
extern "C" void RpcClientTest(int32_t* progress);
...
AudioCallbackInfo AudioModule::audiocallbackinfo_;
...
//..............
JSIValue AudioModule::Oncallback(const JSIValue thisVal, const JSIValue *args, uint8_t argsSize)
{
    TRACE(0, "%s %d", __func__, __LINE__);
   audiocallbackinfo_.callback = JSI::AcquireValue(args[0]);
   audiocallbackinfo_.thisVal_ = JSI::AcquireValue(thisVal);

   return JSI::CreateUndefined();
}

JSIValue AudioModule::PlayRemote(const JSIValue thisVal, const JSIValue *args, uint8_t argsSize)
{
    TRACE(0, "%s %d", __func__, __LINE__);
    if (!JSI::ValueIsNumber(args[0])){
       return JSI::CreateUndefined();
    }
    int32_t data = (int32_t)JSI::ValueToNumber(args[0]);
    TRACE(0, "this progress: %d", data);
    RpcClientTest(&data);
    return JSI::CreateUndefined();
}

void AudioModule::OnPlayRemote(int32_t data)
{
    TRACE(0, "%s %d", __func__, __LINE__);
    JSIValue dataOne = JSI::CreateObject();

    JSI::SetNumberProperty(dataOne, "progress", (double)data);

    JSIValue param[1] = {dataOne};

    FuncParams* params = new FuncParams();
    params->handlerRef = audiocallbackinfo_.callback;
    params->thisVarRef_ = audiocallbackinfo_.thisVal_; 
    params->args = param;
    params->argsSize = 1;
    JsAsyncWork::DispatchAsyncWork(OnEventAsyncWorkFunc, reinterpret_cast<void *>(params));
}

void AudioModule::OnEventAsyncWorkFunc(void *data)
{
    TRACE(0, "%s %d", __func__, __LINE__);
...
void InitAudioModule(JSIValue exports)
{
    ...
    JSI::SetModuleAPI(exports, "onCallback", AudioModule::Oncallback);
    JSI::SetModuleAPI(exports, "playRemote", AudioModule::PlayRemote);
