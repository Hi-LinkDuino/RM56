    ...
    uint8_t *ptrParam, uint32_t paramLen)
{
    ...
    cmd_len = (uint32_t)THROUGHPUT_CMD_PAYLOAD_HEADER_LEN + app_through_put_payload.paramLen;
    app_ai_if_custom_ui_send_cmd((uint8_t *)&app_through_put_payload, cmd_len);
...
uint32_t app_throughput_cmd_send_done(void *param1, uint32_t param2, uint8_t ai_index, uint8_t dest_id)
{
    if (throughputTestEnv.isThroughputTestOn && \
            ((app_ai_get_transport_type(ai_index, throughputTestEnv.testAiIndex) == AI_TRANSPORT_BLE) ||\
...
static void app_throughput_test_config_handler(APP_THROUGHPUT_CMD_CODE_E funcCode, uint8_t* ptrParam, uint32_t paramLen, uint8_t ai_index)
{
    ...
    {
        TRACE(4,"throughput_test_config packet_size fail %d %d", ai_mtu, throughputTestConfig.dataPacketSize);
        return;
    }
