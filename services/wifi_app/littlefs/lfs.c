...
int lfs_mkdir(lfs_t *lfs, const char *path) {
    LFS_TRACE(0, "lfs_mkdir(%p, \"%s\")", (void*)lfs, path);
    // deorphan if we haven't yet, needed at most once after poweron
    int err = lfs_fs_forceconsistency(lfs);
    if (err) {
        LFS_TRACE(0, "lfs_mkdir -> %d", err);
        return err;
    }
    ...
    if (!(err == LFS_ERR_NOENT && id != 0x3ff)) {
        LFS_TRACE(0, "lfs_mkdir -> %d", (err < 0) ? err : LFS_ERR_EXIST);
    ...
    if (nlen > lfs->name_max) {
        LFS_TRACE(0, "lfs_mkdir -> %d", LFS_ERR_NAMETOOLONG);
    ...
    LFS_TRACE(0, "lfs_mkdir -> %d", 0);
    return 0;
}

int lfs_dir_open(lfs_t *lfs, lfs_dir_t *dir, const char *path) {
    LFS_TRACE(0, "lfs_dir_open(%p, %p, \"%s\")", (void*)lfs, (void*)dir, path);
    lfs_stag_t tag = lfs_dir_find(lfs, &dir->m, &path, NULL);
    if (tag < 0) {
        LFS_TRACE(0, "lfs_dir_open -> %d", tag);
        return tag;
    }

    if (lfs_tag_type3(tag) != LFS_TYPE_DIR) {
        LFS_TRACE(0, "lfs_dir_open -> %d", LFS_ERR_NOTDIR);
        return LFS_ERR_NOTDIR;
    }
    ...
    if (lfs_tag_id(tag) == 0x3ff) {
        ...
        if (res < 0) {
            LFS_TRACE(0, "lfs_dir_open -> %d", res);
            return res;
        }
    ...
    if (err) {
        LFS_TRACE(0, "lfs_dir_open -> %d", err);
        return err;
    }
    ...
    LFS_TRACE(0, "lfs_dir_open -> %d", 0);
    return 0;
}

int lfs_dir_close(lfs_t *lfs, lfs_dir_t *dir) {
    LFS_TRACE(0, "lfs_dir_close(%p, %p)", (void*)lfs, (void*)dir);
    ...
    LFS_TRACE(0, "lfs_dir_close -> %d", 0);
    return 0;
}

int lfs_dir_read(lfs_t *lfs, lfs_dir_t *dir, struct lfs_info *info) {
    LFS_TRACE(0, "lfs_dir_read(%p, %p, %p)",
    ...
    if (dir->pos == 0) {
        ...
        strcpy(info->name, ".");
        dir->pos += 1;
        LFS_TRACE(0, "lfs_dir_read -> %d", true);
        return true;
    } else if (dir->pos == 1) {
        info->type = LFS_TYPE_DIR;
        strcpy(info->name, "..");
    ...
    while (true) {
        if (dir->id == dir->m.count) {
            if (!dir->m.split) {
                LFS_TRACE(0, "lfs_dir_read -> %d", false);
                return false;
            }

            int err = lfs_dir_fetch(lfs, &dir->m, dir->m.tail);
            if (err) {
                LFS_TRACE(0, "lfs_dir_read -> %d", err);
                return err;
            }
...
int lfs_dir_seek(lfs_t *lfs, lfs_dir_t *dir, lfs_off_t off) {
    LFS_TRACE(0, "lfs_dir_seek(%p, %p, %"PRIu32")",
            (void*)lfs, (void*)dir, off);
    // simply walk from head dir
    int err = lfs_dir_rewind(lfs, dir);
    if (err) {
        LFS_TRACE(0, "lfs_dir_seek -> %d", err);
        return err;
    }
    ...
    while (off != 0) {
        ...
        if (dir->id == dir->m.count) {
            if (!dir->m.split) {
                LFS_TRACE(0, "lfs_dir_seek -> %d", LFS_ERR_INVAL);
    ...
    LFS_TRACE(0, "lfs_dir_seek -> %d", 0);
    return 0;
}

lfs_soff_t lfs_dir_tell(lfs_t *lfs, lfs_dir_t *dir) {
    LFS_TRACE(0, "lfs_dir_tell(%p, %p)", (void*)lfs, (void*)dir);
    (void)lfs;
    LFS_TRACE(0, "lfs_dir_tell -> %"PRId32, dir->pos);
    return dir->pos;
}

int lfs_dir_rewind(lfs_t *lfs, lfs_dir_t *dir) {
    LFS_TRACE(0, "lfs_dir_rewind(%p, %p)", (void*)lfs, (void*)dir);
    // reload the head dir
    int err = lfs_dir_fetch(lfs, &dir->m, dir->head);
    if (err) {
        LFS_TRACE(0, "lfs_dir_rewind -> %d", err);
        return err;
    }
    ...
    dir->id = 0;
    dir->pos = 0;
    LFS_TRACE(0, "lfs_dir_rewind -> %d", 0);
    return 0;
}
...
int lfs_file_opencfg(lfs_t *lfs, lfs_file_t *file,
    ...
    LFS_TRACE(0, "lfs_file_opencfg(%p, %p, \"%s\", %x, %p {"
    ...
    if ((flags & 3) != LFS_O_RDONLY) {
        int err = lfs_fs_forceconsistency(lfs);
        if (err) {
            LFS_TRACE(0, "lfs_file_opencfg -> %d", err);
            return err;
        }
    }
    ...
    LFS_TRACE(0, "lfs_file_opencfg -> %d", 0);
    return 0;
...
int lfs_file_open(lfs_t *lfs, lfs_file_t *file,
        const char *path, int flags) {
    LFS_TRACE(0, "lfs_file_open(%p, %p, \"%s\", %x)",
            (void*)lfs, (void*)file, path, flags);
    static const struct lfs_file_config defaults = {0};
    int err = lfs_file_opencfg(lfs, file, path, flags, &defaults);
    LFS_TRACE(0, "lfs_file_open -> %d", err);
    return err;
}

int lfs_file_close(lfs_t *lfs, lfs_file_t *file) {
    LFS_TRACE(0, "lfs_file_close(%p, %p)", (void*)lfs, (void*)file);
    ...
    LFS_TRACE(0, "lfs_file_close -> %d", err);
    return err;
}
...
int lfs_file_sync(lfs_t *lfs, lfs_file_t *file) {
    LFS_TRACE(0, "lfs_file_sync(%p, %p)", (void*)lfs, (void*)file);
    LFS_ASSERT(file->flags & LFS_F_OPENED);
    ...
    while (true) {
        ...
        if (err) {
            file->flags |= LFS_F_ERRED;
            LFS_TRACE(0, "lfs_file_sync -> %d", err);
            return err;
        }
...
lfs_ssize_t lfs_file_read(lfs_t *lfs, lfs_file_t *file,
        void *buffer, lfs_size_t size) {
    LFS_TRACE(0, "lfs_file_read(%p, %p, %p, %"PRIu32")",
    ...
    if (file->flags & LFS_F_WRITING) {
        ...
        int err = lfs_file_flush(lfs, file);
        if (err) {
            LFS_TRACE(0, "lfs_file_read -> %"PRId32, err);
            return err;
        }
    }
    ...
    if (file->pos >= file->ctz.size) {
        // eof if past end
        LFS_TRACE(0, "lfs_file_read -> %"PRId32, 0);
        return 0;
    }
    ...
    LFS_TRACE(0, "lfs_file_read -> %"PRId32, size);
    return size;
}

lfs_ssize_t lfs_file_write(lfs_t *lfs, lfs_file_t *file,
        const void *buffer, lfs_size_t size) {
    LFS_TRACE(0, "lfs_file_write(%p, %p, %p, %"PRIu32")",
            (void*)lfs, (void*)file, buffer, size);
    LFS_ASSERT(file->flags & LFS_F_OPENED);
    ...
    if (file->flags & LFS_F_READING) {
        ...
        int err = lfs_file_flush(lfs, file);
        if (err) {
            LFS_TRACE(0, "lfs_file_write -> %"PRId32, err);
            return err;
        }
    }
    ...
    if (file->pos + size > lfs->file_max) {
        ...
        LFS_TRACE(0, "lfs_file_write -> %"PRId32, LFS_ERR_FBIG);
        return LFS_ERR_FBIG;
    }
    ...
    if (!(file->flags & LFS_F_WRITING) && file->pos > file->ctz.size) {
        ...
        while (file->pos < pos) {
            ...
            if (res < 0) {
                LFS_TRACE(0, "lfs_file_write -> %"PRId32, res);
                return res;
            }
        }
    }
...
relocate:
    ...
    file->flags &= ~LFS_F_ERRED;
    LFS_TRACE(0, "lfs_file_write -> %"PRId32, size);
    return size;
}

lfs_soff_t lfs_file_seek(lfs_t *lfs, lfs_file_t *file,
        lfs_soff_t off, int whence) {
    LFS_TRACE(0, "lfs_file_seek(%p, %p, %"PRId32", %d)",
    ...
    int err = lfs_file_flush(lfs, file);
    if (err) {
        LFS_TRACE(0, "lfs_file_seek -> %"PRId32, err);
        return err;
    }
    ...
    if (npos > lfs->file_max) {
        ...
        LFS_TRACE(0, "lfs_file_seek -> %"PRId32, LFS_ERR_INVAL);
        return LFS_ERR_INVAL;
    }

    // update pos
    file->pos = npos;
    LFS_TRACE(0, "lfs_file_seek -> %"PRId32, npos);
    return npos;
}

int lfs_file_truncate(lfs_t *lfs, lfs_file_t *file, lfs_off_t size) {
    LFS_TRACE(0, "lfs_file_truncate(%p, %p, %"PRIu32")",
            (void*)lfs, (void*)file, size);
    LFS_ASSERT(file->flags & LFS_F_OPENED);
    LFS_ASSERT((file->flags & 3) != LFS_O_RDONLY);

    if (size > LFS_FILE_MAX) {
        LFS_TRACE(0, "lfs_file_truncate -> %d", LFS_ERR_INVAL);
        return LFS_ERR_INVAL;
    }
    ...
    if (size < oldsize) {
        ...
        int err = lfs_file_flush(lfs, file);
        if (err) {
            LFS_TRACE(0, "lfs_file_truncate -> %d", err);
            return err;
        }
        ...
        if (file->pos != oldsize) {
            ...
            if (res < 0) {
                LFS_TRACE(0, "lfs_file_truncate -> %d", res);
                return (int)res;
            }
        }
    ...
    LFS_TRACE(0, "lfs_file_truncate -> %d", 0);
    return 0;
}

lfs_soff_t lfs_file_tell(lfs_t *lfs, lfs_file_t *file) {
    LFS_TRACE(0, "lfs_file_tell(%p, %p)", (void*)lfs, (void*)file);
    LFS_ASSERT(file->flags & LFS_F_OPENED);
    (void)lfs;
    LFS_TRACE(0, "lfs_file_tell -> %"PRId32, file->pos);
    return file->pos;
}

int lfs_file_rewind(lfs_t *lfs, lfs_file_t *file) {
    LFS_TRACE(0, "lfs_file_rewind(%p, %p)", (void*)lfs, (void*)file);
    lfs_soff_t res = lfs_file_seek(lfs, file, 0, LFS_SEEK_SET);
    if (res < 0) {
        LFS_TRACE(0, "lfs_file_rewind -> %d", res);
        return (int)res;
    }

    LFS_TRACE(0, "lfs_file_rewind -> %d", 0);
    return 0;
}

lfs_soff_t lfs_file_size(lfs_t *lfs, lfs_file_t *file) {
    LFS_TRACE(0, "lfs_file_size(%p, %p)", (void*)lfs, (void*)file);
    LFS_ASSERT(file->flags & LFS_F_OPENED);
    (void)lfs;
    if (file->flags & LFS_F_WRITING) {
        LFS_TRACE(0, "lfs_file_size -> %"PRId32,
                lfs_max(file->pos, file->ctz.size));
        return lfs_max(file->pos, file->ctz.size);
    } else {
        LFS_TRACE(0, "lfs_file_size -> %"PRId32, file->ctz.size);
...
int lfs_stat(lfs_t *lfs, const char *path, struct lfs_info *info) {
    LFS_TRACE(0, "lfs_stat(%p, \"%s\", %p)", (void*)lfs, path, (void*)info);
    lfs_mdir_t cwd;
    lfs_stag_t tag = lfs_dir_find(lfs, &cwd, &path, NULL);
    if (tag < 0) {
        LFS_TRACE(0, "lfs_stat -> %d", tag);
        return (int)tag;
    }

    int err = lfs_dir_getinfo(lfs, &cwd, lfs_tag_id(tag), info);
    LFS_TRACE(0, "lfs_stat -> %d", err);
    return err;
}

int lfs_remove(lfs_t *lfs, const char *path) {
    LFS_TRACE(0, "lfs_remove(%p, \"%s\")", (void*)lfs, path);
    // deorphan if we haven't yet, needed at most once after poweron
    int err = lfs_fs_forceconsistency(lfs);
    if (err) {
        LFS_TRACE(0, "lfs_remove -> %d", err);
        return err;
    }
    ...
    if (tag < 0 || lfs_tag_id(tag) == 0x3ff) {
        LFS_TRACE(0, "lfs_remove -> %d", (tag < 0) ? tag : LFS_ERR_INVAL);
    ...
    if (lfs_tag_type3(tag) == LFS_TYPE_DIR) {
        ...
        if (dir.count > 0 || dir.split) {
            LFS_TRACE(0, "lfs_remove -> %d", LFS_ERR_NOTEMPTY);
    ...
    LFS_TRACE(0, "lfs_remove -> %d", 0);
    return 0;
}

int lfs_rename(lfs_t *lfs, const char *oldpath, const char *newpath) {
    LFS_TRACE(0, "lfs_rename(%p, \"%s\", \"%s\")", (void*)lfs, oldpath, newpath);

    // deorphan if we haven't yet, needed at most once after poweron
    int err = lfs_fs_forceconsistency(lfs);
    if (err) {
        LFS_TRACE(0, "lfs_rename -> %d", err);
        return err;
    }
    ...
    if (oldtag < 0 || lfs_tag_id(oldtag) == 0x3ff) {
        LFS_TRACE(0, "lfs_rename -> %d", (oldtag < 0) ? oldtag : LFS_ERR_INVAL);
    ...
    if ((prevtag < 0 || lfs_tag_id(prevtag) == 0x3ff) &&
        ...
        LFS_TRACE(0, "lfs_rename -> %d", (prevtag < 0) ? prevtag : LFS_ERR_INVAL);
    ...
    if (prevtag == LFS_ERR_NOENT) {
        ...
        if (nlen > lfs->name_max) {
            LFS_TRACE(0, "lfs_rename -> %d", LFS_ERR_NAMETOOLONG);
            return LFS_ERR_NAMETOOLONG;
        }
    } else if (lfs_tag_type3(prevtag) != lfs_tag_type3(oldtag)) {
        LFS_TRACE(0, "lfs_rename -> %d", LFS_ERR_ISDIR);
        ...
        if (prevdir.count > 0 || prevdir.split) {
            LFS_TRACE(0, "lfs_rename -> %d", LFS_ERR_NOTEMPTY);
            return LFS_ERR_NOTEMPTY;
        }
    ...
    LFS_TRACE(0, "lfs_rename -> %d", 0);
    return 0;
}

lfs_ssize_t lfs_getattr(lfs_t *lfs, const char *path,
        uint8_t type, void *buffer, lfs_size_t size) {
    LFS_TRACE(0, "lfs_getattr(%p, \"%s\", %"PRIu8", %p, %"PRIu32")",
            (void*)lfs, path, type, buffer, size);
    lfs_mdir_t cwd;
    lfs_stag_t tag = lfs_dir_find(lfs, &cwd, &path, NULL);
    if (tag < 0) {
        LFS_TRACE(0, "lfs_getattr -> %"PRId32, tag);
        return tag;
    }
    ...
    if (id == 0x3ff) {
        ...
        if (err) {
            LFS_TRACE(0, "lfs_getattr -> %"PRId32, err);
            return err;
        }
    }
    ...
    if (tag < 0) {
        if (tag == LFS_ERR_NOENT) {
            LFS_TRACE(0, "lfs_getattr -> %"PRId32, LFS_ERR_NOATTR);
    ...
    LFS_TRACE(0, "lfs_getattr -> %"PRId32, size);
    return size;
}
...
int lfs_setattr(lfs_t *lfs, const char *path,
        uint8_t type, const void *buffer, lfs_size_t size) {
    LFS_TRACE(0, "lfs_setattr(%p, \"%s\", %"PRIu8", %p, %"PRIu32")",
            (void*)lfs, path, type, buffer, size);
    if (size > lfs->attr_max) {
        LFS_TRACE(0, "lfs_setattr -> %d", LFS_ERR_NOSPC);
        return LFS_ERR_NOSPC;
    }

    int err = lfs_commitattr(lfs, path, type, buffer, size);
    LFS_TRACE(0, "lfs_setattr -> %d", err);
    return err;
}

int lfs_removeattr(lfs_t *lfs, const char *path, uint8_t type) {
    LFS_TRACE(0, "lfs_removeattr(%p, \"%s\", %"PRIu8")", (void*)lfs, path, type);
    int err = lfs_commitattr(lfs, path, type, NULL, 0x3ff);
    LFS_TRACE(0, "lfs_removeattr -> %d", err);
    return err;
}
...
int lfs_format(lfs_t *lfs, const struct lfs_config *cfg) {
    LFS_TRACE(0, "lfs_format(%p, %p {.context=%p, "
...
int lfs_mount(lfs_t *lfs, const struct lfs_config *cfg) {
    LFS_TRACE(0, "lfs_mount(%p, %p {.context=%p, "
                ".read=%p, .prog=%p, .erase=%p, .sync=%p, "
                ".read_size=%"PRIu32", .prog_size=%"PRIu32", "
                ".block_size=%"PRIu32", .block_count=%"PRIu32", "
                ".block_cycles=%"PRIu32", .cache_size=%"PRIu32", "
...
int lfs_unmount(lfs_t *lfs) {
    LFS_TRACE(0, "lfs_unmount(%p)", (void*)lfs);
    int err = lfs_deinit(lfs);
    LFS_TRACE(0, "lfs_unmount -> %d", err);
    return err;
}
...
int lfs_fs_traverse(lfs_t *lfs,
    ...
    LFS_TRACE(0, "lfs_fs_traverse(%p, %p, %p)",
    ...
    if (lfs->lfs1) {
        ...
        if (err) {
            LFS_TRACE(0, "lfs_fs_traverse -> %d", err);
            return err;
        }
    ...
    while (!lfs_pair_isnull(dir.tail)) {
        ...
        for (uint16_t id = 0; id < dir.count; id++) {
            ...
            if (tag < 0) {
                ...
                LFS_TRACE(0, "lfs_fs_traverse -> %d", tag);
                return tag;
            }
            lfs_ctz_fromle32(&ctz);
    ...
    LFS_TRACE(0, "lfs_fs_traverse -> %d", 0);
    return 0;
}
...
lfs_ssize_t lfs_fs_size(lfs_t *lfs) {
    LFS_TRACE(0, "lfs_fs_size(%p)", (void*)lfs);
    lfs_size_t size = 0;
    int err = lfs_fs_traverse(lfs, lfs_fs_size_count, &size);
    if (err) {
        LFS_TRACE(0, "lfs_fs_size -> %"PRId32, err);
        return err;
    }
...
int lfs_migrate(lfs_t *lfs, const struct lfs_config *cfg) {
    LFS_TRACE(0, "lfs_migrate(%p, %p {.context=%p, "
                ".read=%p, .prog=%p, .erase=%p, .sync=%p, "
                ".read_size=%"PRIu32", .prog_size=%"PRIu32", "
                ".block_size=%"PRIu32", .block_count=%"PRIu32", "
                ".block_cycles=%"PRIu32", .cache_size=%"PRIu32", "
