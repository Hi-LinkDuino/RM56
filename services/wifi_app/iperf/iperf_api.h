...
#define Psctp 12
#define DEFAULT_UDP_BLKSIZE (1470) /* default is dynamically set, else this */
#define DEFAULT_TCP_BLKSIZE (1460)//(8 * 1024)  /* default read/write block size */
...
int	iperf_get_test_repetition( struct iperf_test* ipt );
...
void	iperf_set_test_repetition( struct iperf_test* ipt, int one_off );
...
/**
 * iperf_free_test -- nts_free resources used by test, calls iperf_free_stream to
 * nts_free streams
 *
 */
...
/**
 * iperf_free_stream -- nts_free resources associated with test
 *
 */
...
#define iperf_printf(format, ...) \
			do {\
				printf(format, ##__VA_ARGS__);\
				if(format[strlen(format)-1] != '\n') \
					printf("\n");\
			} while (0)
#define iperf_err(format, ...) \
			do {\
				printf(format, ##__VA_ARGS__);\
				if(format[strlen(format)-1] != '\n') \
					printf("\n");\
			} while (0)
#define iperf_errexit(format, ...) \
			do {\
				printf(format, ##__VA_ARGS__);\
				if(format[strlen(format)-1] != '\n') \
					printf("\n");\
